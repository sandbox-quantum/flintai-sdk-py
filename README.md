...
- **LLM SDK wrapping** — route LLM traffic through the FlintAI guardrails proxy with a single `flintai_sdk.wrap()` call. Supports OpenAI, Anthropic, Google GenAI, and LangChain chat models.
...
|---|---|---|
| OpenAI SDK | LLM SDK | `flintai_sdk.wrap()` |
| Anthropic SDK | LLM SDK | `flintai_sdk.wrap()` |
| Google GenAI SDK | LLM SDK | `flintai_sdk.wrap()` |
| LangChain (`ChatOpenAI`, `ChatAnthropic`, `ChatGoogleGenerativeAI`) | LLM SDK | `flintai_sdk.wrap()` |
...
Route all LLM traffic through the FlintAI guardrails proxy. Create your sync client as usual, then call `flintai_sdk.wrap()` — it auto-detects the provider and applies guardrails routing. Async clients (`AsyncOpenAI`, `AsyncAnthropic`) are not supported.
...
Create your LangChain chat model as usual, then call `flintai_sdk.wrap()` — it detects the LangChain model, finds the underlying SDK client, and applies guardrails routing:
...
ADK agents lazily create their GenAI client at runtime and use `generate_content_config` for per-request routing, so `flintai_sdk.wrap()` cannot be used. Use the ADK plugin instead:
...
`flintai_sdk.wrap()` and `flintai_sdk.init()` accept these guardrails parameters:
...
client = flintai_sdk.wrap(client)  # reads from env vars / .env file
```

**Precedence:** Explicit parameters > environment variables > auto-extract from `client.api_key` (for `llm_api_key` in `wrap()` only).

**Multiple provider keys:** If multiple provider API keys are set in the environment (`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`), auto-detection will fail. Pass `provider` explicitly to `flintai_sdk.init()` to disambiguate.

## How It Works

1. `flintai_sdk.wrap(client, gateway_url=..., ...)` auto-detects the provider from the client type, computes the provider-specific path prefix (`/openai`, `/anthropic`, `/gemini`), rewrites the client's base URL, and injects custom headers (`X-FlintAI-API-Key`, `X-LLM-API-Key`, `X-Guardrails-Policy-Id`)
...
**Thread safety:** `flintai_sdk.init()`, `flintai_sdk.wrap()`, and `flintai_sdk.shutdown()` are not thread-safe. Call them from the main thread during application startup. Once initialized, wrapped clients can be used from any thread — the underlying SDK clients handle their own thread safety.
...
# Register after init
flintai_sdk.init()
flintai_sdk.register_plugin(MyPlugin())
```
...
| `on_shutdown()` | `flintai_sdk.shutdown()` is called |
